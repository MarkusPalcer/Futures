...
namespace Futures
{
  ...
  public static partial class Future
  {
    ...
    /// <param name="source">A future to get notification values for.</param>
    /// <returns>
    ...
    /// <summary>
    /// Dematerializes the explicit notifications values back to implicit notifications
    /// 
    /// </summary>
    /// <typeparam name="T">The type of the result of the future.</typeparam>
    /// <param name="source">A future to dematerialize.</param>
    /// <returns>
    /// An future returning executing OnError and OnDone according to the notification returned by the source
    /// </returns>
    /// <exception cref="T:System.ArgumentNullException"><paramref name="source"/> is null.</exception>
    public static IFuture<T> Dematerialize<T>(this IFuture<Notification<T>> source)
    {
      return Create<T>(
        o => source.Subscribe(r =>
        {
          switch (r.Kind)
          {
